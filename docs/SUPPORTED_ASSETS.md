# Satra — Supported Assets & Networks

> **Source of truth:** `docs/SUPPORTED_ASSETS.md` until the asset CSV is added to this repository.
>
> **Scope rule (HARD):** The wallet supports **ONLY** the coins, tokens, and networks listed in this document. Do **not** add, remove, or substitute any asset/network without updating the source asset list first. No "extra" chains (e.g., no Cardano, Cosmos Hub, Fantom, Linea, Mantle, etc.) unless they appear in the source asset list.
>
...
1. **Single source of truth** — this document wins over any in-code list until the source CSV is added to the repository. If a discrepancy appears, treat the supported-assets row as correct and update code/docs to match.
...
10. **Adding/removing assets** — must be source-first: update the source asset list, regenerate this doc, then update code. No silent additions.
